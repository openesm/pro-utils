...
dayjs.extend(utc)          // 转换时区插件
dayjs.extend(timezone)     // 解析时区插件
dayjs.extend(relativeTime) // 相对时间插件

// 设置默认使用北京时区 , America/New_York 美国
...
export function formatWithTz(date: string | number | Date, tz = 'Asia/Shanghai', format?: string): string {
    return dayjs(date).tz(tz).format(format || FORMAT_DEFAULT)
}
